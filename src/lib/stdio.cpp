...
int vsprintf(char *buf, const char *fmt, var_list args)
{
    ...
    int field_width = 0; // 输出字段宽度
    int precision = 0;   // min 整数数字个数；max 字符串中字符个数
    int qualifier = 0;   // 'h', 'l' 或 'L' 用于整数字段
    uint32 num = 0;
    uint8 *ptr = nullptr;
    ...
    repeat:
        ...
        {
            ++fmt;
            // 因此调用 VAR_ARG_FN 取宽度值
            field_width = VAR_ARG_FN(args, int);
        ...
        if (*fmt == '.')
        {
            ...
            {
                // 因此调用 VAR_ARG_FN 取精度值
                precision = VAR_ARG_FN(args, int);
            }
        ...
        case 'c':
            ...
            if (!(flags & LEFT))
                ...
                while (--field_width > 0)
                    *str++ = ' ';
            *str++ = (unsigned char)VAR_ARG_FN(args, int);
            // 如果宽度域还大于 0，则表示为左对齐
        ...
        case 'o':
            num = VAR_ARG_FN(args, unsigned long);
        ...
        case 'n':
            // 首先利用 VAR_ARG_FN() 取得该参数指针
            ip = VAR_ARG_FN(args, int *);
        ...
        case 'f':{
            flags |= SIGN;
            flags |= DOUBLE;
            double dnum = (double)VAR_ARG_FN(args, double);
        ...
        case 'm': { // mac address
            flags |= SMALL | ZEROPAD;
            ptr = (uint8*)VAR_ARG_FN(args, char *);
            for (size_t t = 0; t < 6; t++, ptr++)
            {
                uint32 tempNum = (uint32)*ptr;
                str = number(str, &tempNum, 16, 2, precision, flags);
                *str = ':';
                str++;
            }
            str--;
        ...
        case 'r': { // ip address
            flags |= SMALL;
            ptr = VAR_ARG_FN(args, uint8 *);
            for (size_t t = 0; t < 4; t++, ptr++)
            {
                uint32 tempNum = (uint32)*ptr;
                str = number(str, &tempNum, 10, field_width, precision, flags);
                *str = '.';
                str++;
            }
            str--;
    ...
    *str = '\0';
    ...
    // 返回转换好的字符串长度值
    i = str - buf;
    assert(i < 10240, "buffer overflow");
    return i;
}
